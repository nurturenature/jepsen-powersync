...
final jwksKey = jsonDecode(jwksKeyStr);

/// Public key should match PowerSync's `.env`.
final jwksPublicKeyStr = utf8.decode(base64Decode(
...
final jwksPublicKey = jsonDecode(jwksPublicKeyStr);

final jwksPrivateKeyStr = utf8.decode(base64Decode(
...
final jwksPrivateKey = jsonDecode(jwksPrivateKeyStr);

final jwkPrivateKey = JsonWebKey.fromJson(jwksPrivateKey);

final keyStore = JsonWebKeyStore()..addKey(jwkPrivateKey);
...
Future<String> generateToken() async {
  ...
  builder.setProtectedHeader('alg', jwksKey['alg']);
  builder.setProtectedHeader('kid', jwksKey['kid']);
  builder.jsonContent = claims.toJson();
  builder.addRecipient(jwkPrivateKey, algorithm: jwksKey['alg']);

  final jws = builder.build().toCompactSerialization();

  log.finer(
      'Created token w/payload: ${(await JsonWebSignature.fromCompactSerialization(jws).getPayload(keyStore)).jsonContent}');
